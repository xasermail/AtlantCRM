...
// контроллер
myApp.controller("soprCtrl", function soprCtrl($scope, $http, $interval) {

  var i = 0;
  var j = 0;
  $scope.sopr = [];
  $scope.sopr_item = [];
  $scope.sopr_srochno_item = {};
  $scope.comment = [];
  $scope.page = 1;
  $scope.vsego = 0;
  $scope.last_page = 0;
  ...
  // событие при открытии вкладки
  $scope.$on("global.selectedSoprSubMenuItemChanged", function selectedSoprSubMenuItemChanged(event, newValue) {
    if ($scope.global.selectedMenuItem === "menuItemSopr") {
  ...
  $scope.refreshData = function refreshData() {
    ...
    }).then(function soprGetDataSuccess(data) {

      $scope.sopr = data.data;

      if ($scope.sopr.length !== 0) {
        $scope.sopr_item = $scope.sopr[0];
        $scope.vsego = $scope.sopr[0].CNT;
        $scope.last_page = $scope.sopr[0].CNT_PAGE;
    ...
    }, function soprGetDataFailed(err) {
  ...
  $scope.saveSopr = function saveSopr(item) {
    $scope.global.showWaitingForm("Сохранение данных...");
    $http({
      "method": "POST",
      "url": "/Home/SaveSopr",
      data: item
    }).then(function soprSaveDataSuccess(data) {
    ...
    }, function soprSaveDataFailed(err) {
  ...
  function resizeTd() {
    ...
    $scope.TD4  = Math.trunc((w * 0.06));
    $scope.TD5  = Math.trunc((w * 0.06));
    $scope.TD6  = Math.trunc((w * 0.05));
    $scope.TD7  = Math.trunc((w * 0.045));
    $scope.TD8  = Math.trunc((w * 0.065));
    $scope.TD9  = Math.trunc((w * 0.06));
  ...
  $scope.exportSoprToExcel = function exportSoprToExcel() {

    $scope.global.showWaitingForm("Выгрузка данных...");

    $http({
      "method": "POST",
      "url": "/Home/ExportSoprToExcel",
      data: {
        status: $scope.status
      }
