...
appCtrl = myApp.controller("appCtrl", function appCtrl($scope, $http, $timeout, $interval) {
  ...
  // подменю режима Сопровождение
  $scope.global.selectedSoprSubMenuItem = 0;
  // подменю режима Действующие клиенты
  $scope.global.selectedProdlSubMenuItem = 0;
  // подменю режима Дилер Д
  $scope.global.selectedDilerDSubMenuItem = 0;
  // подменю Скрипты
  $scope.global.selectedScriptsSubMenuItem = 0;
  ...
  // переменная для отображения денежного мешка (план в процентах)
  $scope.global.motiv = {};
  $scope.global.motiv.plan = -1;
  // переменная для отображения пути к рисунку заполняемой монеты (план в процентах)
  $scope.global.motiv.planPhoto = "";
  // переменная для отображения зарплаты от суммы перевыполненного плана
  $scope.global.motiv.over = -1;
  ...
  // "Сам пришел"
  $scope.global.const.M_IST_SAM_PRISHEL = 3;
  ...
  // Статус "Срочно" в режиме "Сопровождение"
  $scope.global.const.SOPR_M_STATUS_ID_SROCHNO = 10;
  // Статус "Задача" в режиме "Сопровождение"
  $scope.global.const.SOPR_M_STATUS_ID_ZADACHA = 4;
  // Статус "Задача" в режиме "Сопровождение"
  $scope.global.const.SOPR_M_STATUS_ID_OTKAZ = 3;
  // Количество дней для сокрытия записи в отчете Не продлил
  $scope.global.const.UCHET_REPORT_NE_HODYAT_SKR_DAYS = 30;
  // Ид справочника M_PRODUCT - для настройки товаров салона
  $scope.global.const.M_MANUAL_ID = 10;
  ...
  // при нажатии на элемент подменю Сопровождение
  $scope.subMenuSoprClickHandler = function subMenuSoprClickHandler(e) {
    var target = e.target;
    var subMenuItemArray = $(target).closest("[data-index]");
    if (subMenuItemArray.length === 1) {
      $scope.global.selectedSoprSubMenuItem = +subMenuItemArray[0].getAttribute("data-index");
    }
  };

  // при нажатии на элемент подменю Действующие клиенты
  $scope.subMenuProdlClickHandler = function subMenuProdlClickHandler(e) {
    var target = e.target;
    var subMenuItemArray = $(target).closest("[data-index]");
    if (subMenuItemArray.length === 1) {
      $scope.global.selectedProdlSubMenuItem = +subMenuItemArray[0].getAttribute("data-index");
    }
  };

  // при нажатии на элемент подменю Дилер Д
  $scope.subMenuDilerDClickHandler = function subMenuDilerDClickHandler(e) {
    var target = e.target;
    var subMenuItemArray = $(target).closest("[data-index]");
    if (subMenuItemArray.length === 1) {
      $scope.global.selectedDilerDSubMenuItem = +subMenuItemArray[0].getAttribute("data-index");
    }
  };
  ...
  $scope.global.waitingFormShown = true;
  ...
  $scope.global.hideWaitingForm = function hideWaitingForm() {
    ...
    if ($scope.waitingFormCallCnt === 0 || $scope.waitingFormCallCnt < 0) {
      $scope.global.waitingFormShown = false;
    }
  ...
  // Информационное сообщение alert
  // показать
  $scope.infoAlertShown = false;
  $scope.global.showInfoAlert = function showInfoAlert(msg) {
    $scope.infoAlertMsg = msg;
    $scope.infoAlertShown = true;
    // если было открыто окно ожидания, его надо закрыть
    $scope.waitingFormCallCnt = 0;
    $scope.global.hideWaitingForm();
  };
  //
  // скрыть
  $scope.global.hideInfoAlert = function hideInfoAlert() {
    $scope.infoAlertShown = false;
  };
  // /информационное сообщение alert
  ...
  // открыть режим Учет - Продажа для переданного O_ANK_ID
  $scope.global.openUchetSkladRas = function openUchetSkladRas(O_ANK_ID) {
    // посылаю намерение открыть режим Учёт - Продажа, чтобы 
    // само открытие произошло в uchetSkladRasCtrl
    $scope.$broadcast("openUchetSkladRas", O_ANK_ID);
  };

  
  // открыть режим Учет - Продажа для переданного O_SKLAD_RAS_ID
  ...
  $scope.loadManuals = function loadManuals(manualName) {
    ...
    if (manualName == null) {
      ...
      manualAllName.push("O_NASTR"); // настройки салона
      manualAllName.push("M_SMS_TEMPLATE_TYPE"); // типы шаблонов для автоматической рассылки СМС

      manualAllName.push("M_PRODUCT_ALL");  // тип оборудования, полный справочник, без отметок по салону
      manualAllName.push("M_PRODUCT_ORG");  // тип оборудования, отмеченные по салону товары
  ...
  // контроллерам для обновления их состояния
    ...
    // при изменении подвкладки в меню Сопровождение отсылаю событие всем заинтересованным
    // контроллерам для обновления их состояния
  }).then(function selectedSoprSubMenuItemChangedSuccess() {

    $scope.$watch("global.selectedSoprSubMenuItem", (newValue) => {
      $scope.$broadcast("global.selectedSoprSubMenuItemChanged", newValue);
      $scope.pushHistory();
    });

    return new Promise(function (resolve) { resolve(1); });

    // при изменении подвкладки в меню Действующие клиенты отсылаю событие всем заинтересованным
    // контроллерам для обновления их состояния
  }).then(function selectedProdlSubMenuItemChangedSuccess() {

    $scope.$watch("global.selectedProdlSubMenuItem", (newValue) => {
      $scope.$broadcast("global.selectedProdlSubMenuItemChanged", newValue);
      $scope.pushHistory();
    });

    return new Promise(function (resolve) { resolve(1); });

    // при изменении подвкладки в меню Дилер Д отсылаю событие всем заинтересованным
    // контроллерам для обновления их состояния
  }).then(function selectedDilerDSubMenuItemChangedSuccess() {

    $scope.$watch("global.selectedDilerDSubMenuItem", (newValue) => {
      $scope.$broadcast("global.selectedDilerDSubMenuItemChanged", newValue);
      $scope.pushHistory();
    });
  ...
  // считаю, что вся программа загрузилась
    ...
    $scope.pushHistory = function pushHistory() {
      ...
      if (
        ...
        history.state[9].val != $scope.global.selectedSoprSubMenuItem||
        history.state[10].val != $scope.global.selectedProdlSubMenuItem||
        history.state[11].val != $scope.global.selectedDilerDlSubMenuItem
      ) {
        history.pushState(
          [
            ...
            { prop: "global.selectedSoprSubMenuItem", val: $scope.global.selectedSoprSubMenuItem },
            { prop: "global.selectedProdlSubMenuItem", val: $scope.global.selectedProdlSubMenuItem },
            { prop: "global.selectedDilerDSubMenuItem", val: $scope.global.selectedDilerDSubMenuItem }
          ],
          null, "#"
        );
      }
  ...
  // при нажатии на меню Учёт - Продажа
  ...
  $scope.global.function.printAnk = function printAnk(ID, ank, newWindow) {
    ...
    if (ank != null) {
      ...
      w.printData.ID_CODE = ank.ID_CODE;
  ...
  // открывает Общение
  $scope.global.function.openDialog = function openDialog() {
  ...
  // проверяет выполненный план
  $scope.global.function.updatePlan = function updatePlan() {
    $http({
      "method": "GET",
      "url": "/Home/GetPlan"
    }).then(function updatePlanSuccess(data) {
      $scope.global.motiv = {};
      $scope.global.motiv.plan = data.data.plan;
      $scope.global.motiv.planPhoto = data.data.plan_photo;
      $scope.global.motiv.over = data.data.over;
    });
  };
