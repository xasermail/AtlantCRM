...
.controller("vipisCtrl", function vipisCtrl($scope, $http, $parse, $sce) {
  ...
  $scope.refresh = function refresh() {
    ...
    $http({
      ...
      $scope.YOUTUBE_URL = null;
      $scope.setVideoDo($scope.LINK);

      $scope.LINK_POSLE = data.data.LINK_POSLE;
      $scope.YOUTUBE_URL_POSLE = null;
      $scope.setVideoPosle($scope.LINK_POSLE);
  ...
  $scope.dobInf = function dobInf() {
    if ($scope.global.ank.ID === 0) {
      $scope.global.showErrorAlert("Сохраните анкету перед добавлением информации о результатах");
      return false;
    }
    ...
    $http({
      ...
      // идентификатор созданной Результаты
  ...
  $scope.setVideoDo = function setVideoDo(link) {
    if (link != null) {
  ...
  $scope.setVideoPosle = function setVideoPosle(link) {
    if (link != null) {
      link = link.replace("https://www.youtube.com/watch?v=", "");
      link = link.replace("https://youtu.be/", "");
      $scope.YOUTUBE_URL_POSLE = $sce.trustAsResourceUrl("https://www.youtube.com/embed/" + link);
    } else {
      $scope.YOUTUBE_URL_POSLE = null;
    }
  }
