...
kontListByModeCtrl = myApp.controller("kontListByModeCtrl", function kontListByModeCtrl($scope, $http) {
  ...
  // маска телефона
  $("#kont.kont-no-came-container .kont-edit .phone-mobile").mask("+7(999) 999-99-99");
  ...
  // обновление данных вкладки Не пришедшие
    ...
    $scope.getDataNotCame = function getDataNotCame() {
      ...
      $http({
          ...
          params:
          {
            ...
            o_kont_ank_id_search: $scope.global.searchO_KONT_ANK_ID,
    ...
    $scope.btnChangeSeansClick = function btnChangeSeansClick(item) {
      closeAllWindow();
      // права на запись
      ...
      $scope.changedKontSeansID = parseInt(item.KONT_SEANS_ID);
      //сохраняем все параметры, т.к. предпологается редактирование записи
      $scope.change_m_seans_time_id = parseInt(item.M_SEANS_TIME_ID);
  ...
  // предыдущие 10 страниц
    ...
    // нажатие на ФИО для открытия редакирования
    $scope.tdChangeKontClickHandler = function tdChangeKontClickHandler(e, item) {
      ...
      if (($scope.global.selectedKontSubMenuItem === 1 && $scope.global.function.noHavePravoWrite(7, 41)) ||
          ($scope.global.selectedKontSubMenuItem === 2 && $scope.global.function.noHavePravoWrite(7, 42)) ||
          ($scope.global.selectedKontSubMenuItem === 3 && $scope.global.function.noHavePravoWrite(7, 43)))
      {
        return false;
      }
    ...
    // присвоение класса ячейке Статус, в зависимости от выбранного значения
    $scope.getMKontStatusIdClass = function getMKontStatusIdClass(item) {

      var cl = {};
      var status_name = item.M_KONT_STATUS_ID_NAME.toUpperCase();
      if (status_name === "Отказ".toUpperCase()) {
        cl["status-otkaz-color"] = true;
      } else if (status_name === "Перезвонить".toUpperCase()) {
        cl["status-perezvon-color"] = true;
      } else if (status_name === "Срочно".toUpperCase()) {
        cl["status-sroch-color"] = true;
      } else if (status_name === "Зв.позже".toUpperCase()) {
        cl["status-zn-pozje-color"] = true;
      }
    ...
    // добавить нового человека в Не пришедшие
    $scope.btnDobClick = function btnDobClick(e) {
      ...
      $scope.kont.O_KONT_SEANS_COMMENTList = [];

      // на какую дату запишется этот Не пришедший, записываю на вчера
      var d = new Date();
      d.setDate(d.getDate() - 1);
      // будет в виде 2018-03-29
      var m = "" + (d.getMonth() + 1);
      if (m.length === 1) {
        m = "0" + m;
      }
      var yearMonthDay = d.getFullYear() + "-" + m + "-" + d.getDate(); 
      $scope.kont.yearMonthDay = yearMonthDay;

      // на какое время, на первое попавшееся
      $scope.kont.M_SEANS_TIME_ID = $scope.global.manual.M_SEANS_TIME[0].ID;
      $scope.kont.SOZD_NEPR = 1;

    };


    // сохранить новый контакт в Не пришедших
    $scope.kontEditSave = () => {
      scopeKontCtrl.doKontEditSave($scope);
    };

    // закрыть окно создания нового контакта в Не пришедших
    $scope.kontEditClose = () => {
    ...
    // это нужно для сохранения нового контакта, при создании Не пришедшего просто обновление
