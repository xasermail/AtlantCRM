...
myApp.controller("uchetSkladRasDocCtrl", function uchetSkladPrCtrl($scope, $http) {
  ...
  $scope.initPage = function initPage() {
    ...
    $scope.o_ras_doc = {};
    // дата счета
    $scope.o_ras_doc.D_SCHET = $scope.global.function.newDateNoTime();
    // номер счета
  ...
  $scope.reCalcRashod = function reCalcRashod() {
    try {
      ...
      $scope.o_ras_doc.KOLVO = 1;
      $scope.o_ras_doc.SUMMA_RASH = $scope.o_ras_doc.RASHOD;
    } catch (e) {
    }
  };

  $scope.reCalcRashodSumma = function reCalcRashodSumma() {
    try {
      var rashod = parseFloat($scope.o_ras_doc.RASHOD);
      var summa = parseFloat($scope.o_ras_doc.SUMMA_VID);

      $scope.o_ras_doc.VOZVR = Math.round((summa - rashod) * 100) / 100;
      $scope.o_ras_doc.KOLVO = 1;
      $scope.o_ras_doc.SUMMA_RASH = rashod;
