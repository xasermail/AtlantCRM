...
myApp.controller("uchetSkladRasCtrl", function uchetSkladRasCtrl($scope, $http, $timeout) {
  ...
  $scope.refresh = function refresh(page, O_SKLAD_RAS_ID) {
    ...
    return $http({
      ...
      if ($scope.uvedoml.d_sob1 != null) {
        $scope.uvedoml1Disabled = true;
      }
  ...
  $scope.btnSaveClick = function btnSaveClick() {
    ...
    // для уведомления 1 надо либо не заполнять ничего, либо заполнять и 
    // вид уведомления и дату уведомления
    if (($scope.uvedoml1Disabled === false) &&
        ($scope.uvedoml.m_vid_sob_id != null && $scope.uvedoml.d_sob1 == null ||
         $scope.uvedoml.m_vid_sob_id == null && $scope.uvedoml.d_sob1 != null)) {
    ...
    //#375
    /* не уверен, что это работает, к тому же это дублирует функционал, который
       выполнен ниже, и который похоже теперь не нужен
    if (($scope.o_sklad_ras.IS_VID == 1) &&
        ($scope.uvedoml.m_vid_sob_id == null || $scope.uvedoml.d_sob1 == null) &&
        ($scope.uvedoml1Disabled === false)) {
      $scope.global.showErrorAlert("Заполните тип события и дату");
      return;
    }
    */
        

    // если заполнены поля уведомления 1 и оно не было уже создано,
    // то проверяю, что у всех позиций есть оплата (частичный платёж)
    if (($scope.uvedoml.d_sob1 != null || $scope.uvedoml.m_vid_sob_id != null) && $scope.uvedoml1Disabled === false) {
      // если у одного из товаров не проставлена частичная оплата,
      // и он не является подарком (стоимость = 0),
      // тогда надо выдать ошибку, потому что уведомление 1 создаётся только
      // после проставления частичной оплаты
      if (
        $scope.productList.find(
            // не подарок
            x => x.o_sklad_ras_product.COST > 0
            // и есть непроставленные частичные оплаты
            && (x.o_sklad_ras_product_opl.find(y => y.OPL == null || y.OPL == 0) != null)
        )
        != null
      ) {
        $scope.global.showErrorAlert("Для создания уведомления (блок \"Добавить звонок\"), необходимо проставить частичную оплату у всех товаров, которые не являются подарками");
        return;
      }
    ...
    // проверяю неправильную последовательность действий пользователя,
    // это происходит, когда уже проставлена галочка "Выдано", но ещё
    // не проставлен Тип события и дата
    if (
      ($scope.uvedoml.m_vid_sob_id == null || $scope.uvedoml.d_sob1 == null)
      &&
