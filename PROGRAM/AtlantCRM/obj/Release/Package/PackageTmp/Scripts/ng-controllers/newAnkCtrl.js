...
newAnkCtrl = myApp.controller("newAnkCtrl", function newAnkCtrl($scope, $http) {
  ...
  $scope.$on("menuItemNewClick", () => {
    ...
    // для отличия новой анкеты с просканированным штрих-кодом, заглушка
    // при сохранении по умолчанию значение null
    $scope.newAnk.GPS_LT = -1;
  ...
  $scope.scan = function scan() {
    ...
    $("#ank-barcode-img").JsBarcode(
      ...
      ("000000000000" + $scope.o_ank_id).substr(($scope.o_ank_id + "").length),
      { width: 3, height: 60, format: "EAN13" }
    );

    $scope.global.ank.ID = $scope.o_ank_id;
  }
});