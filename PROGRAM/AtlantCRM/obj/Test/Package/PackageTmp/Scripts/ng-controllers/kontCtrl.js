...
kontCtrl = myApp.controller("kontCtrl", function kontCtrl($scope, $http) {
  ...
  $(".kont-container div.kont-search input").autocomplete({
    ...
    select: function (event, ui) {
      ...
      if (ui.item.KontOrRek === "KONT") {
        ...
        // если это Не пришедший
        if (ui.item.NEPR === 1) {

          // идентификатор выбранного контакта
          $scope.global.searchO_KONT_ANK_ID = ui.item.id;

          // переключаемся на вкладку Не пришедшие
          // на первую страницу
          scopeKontListByModeCtrl.pageForNotCame = 1;
          if ($scope.global.selectedKontSubMenuItem !== 1) {
            $scope.global.selectedKontSubMenuItem = 1;
          } else {
        ...
        if ($scope.global.selectedKontSubMenuItem !== 3) {
  ...
  $scope.getData = function getData(date) {
    ...
    $scope.global.showWaitingForm("Получение информации..");
  ...
  $scope.tdChangeKontClickHandler = function tdChangeKontClickHandler(kontSeansAndAnk) {
    ...
    $scope.kont.O_KONT_SEANS_COMMENTList = kontSeansAndAnk.O_KONT_SEANS_COMMENTList;
    $scope.kont.yearMonthDay = $scope.yearMonthDay;
    surnameFocus();
    ...
    if (scope.kont.currentMode === "edit") {
      ...
      doKont = $http({
        ...
        params: {
          ...
          date: new Date(scope.kont.yearMonthDay),
          commentForSave: scope.kont.COMMENT,
          M_KONT_STATUS_ID: scope.kont.M_KONT_STATUS_ID,
          M_KONT_IST_ID: scope.kont.M_KONT_IST_ID,
          SOZD_NEPR: scope.kont.SOZD_NEPR
        }
      })
  ...
  $scope.btnChangeSeansClick = function btnChangeSeansClick(dataColumn) {
    ...
    $scope.kontSeans.ID = dataColumn.KontSeansID;
    // сохраняем все параметры, т.к. предполагается редактирование записи
    $scope.kontSeans.M_SEANS_TIME_ID = dataColumn.SeansTimeID;
    $scope.kontSeans.COMMENT = null;
    $scope.kontSeans.SEANS_DATE = $scope.usingDate;
    $scope.kontSeans.O_KONT_SEANS_COMMENTList = dataColumn.O_KONT_SEANS_COMMENTList;
