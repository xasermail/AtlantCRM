...
newZvonokCtrl = myApp.controller("newZvonokCtrl", function newZvonokCtrl($scope, $http, $timeout) {
  ...
  $scope.shablonDelClickHandler = function shablonDel() {
    $scope.global.function.showYesNoDialog("Удалить шаблон?", () => {
  ...
  $scope.saveZvonok = function saveZvonok() {
    if ($scope.global.ank.ID === 0) {
      $scope.global.showErrorAlert("Сохраните анкету перед добавлением информации о звонке");
      return false;
    }
  ...
  $scope.saveSeans = function saveSeans() {
    if ($scope.global.ank.ID === 0) {
      $scope.global.showErrorAlert("Сохраните анкету перед добавлением информации о записи");
      return false;
    }
